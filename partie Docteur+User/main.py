    ...
    with st.sidebar:
        ...
        if st.button("📂 مساعدك الطبي الذكي (SHIFA AI)", width='stretch'):
            try:
                ...
                subprocess.Popen(
                    [sys.executable, "-m", "streamlit", "run", main_app_path, "--server.port", "8502"],
        ...
        if st.button("📂 Parapharmacie", width='stretch'):
            try:
                ...
                subprocess.Popen(
                    [sys.executable, "-m", "streamlit", "run", phar_app_path, "--server.port", "8504"],
                    cwd=str(BASE_DIR)
                )
...
def show_patient_dashboard(db, user_id):
    ...
    with col4:
        if stats['prochain_rdv']:
            rdv_val = stats['prochain_rdv']['date_rdv']
            next_date = rdv_val.strftime('%d/%m/%Y') if hasattr(rdv_val, 'strftime') else str(rdv_val)
        else:
            next_date = "Aucun"
...
def show_chat_interface(db, notif_manager, pdf_gen, user_id):
    ...
    if prompt := st.chat_input("Posez votre question..."):
        ...
        with st.spinner("🤖 Analyse en cours..."):
            reponse = generate_response(prompt, st.session_state.chat_history)
...
def generate_response(prompt, history=None):
    """Génère une réponse médicale via Groq LLM"""
    try:
...
Règles :
1. Réponse structurée et concise (5-8 lignes max)
2. Si l'utilisateur décrit des symptômes :
   - Diagnostic(s) probable(s)
   - Recommandation claire
   - Niveau d'urgence (faible/modéré/élevé/critique)
3. Si la question est générale sur la santé, donne une réponse informative
4. Toujours rappeler : "Cette analyse ne remplace pas une consultation médicale."
5. Si urgence détectée, mentionner le numéro d'urgence Maroc : 141 / SAMU : 15
6. Sois empathique et rassurant"""

        messages = [{"role": "system", "content": system_prompt}]
        
        # Injecter l'historique (max 10 derniers messages)
        if history:
        ...
        logger.error(f"Erreur Groq Chat: {e}")
        return f"⚠️ Erreur de connexion au service IA. Veuillez réessayer.\n\nDétail : {str(e)}"
...
def show_auth_interface(auth_manager):
    ...
    # Style CSS pour l'interface d'authentification - Thème Sombre
    st.markdown("""
        <style>
            /* === FOND GLOBAL AUTH === */
            .stApp, .main, section.main {
